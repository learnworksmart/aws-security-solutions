...
variable "project" {
  default = "ec2-inspec-scan"
}

#the github token for accessing the inspec scripts stored in github.
...
#cron schedule for cloudwatch rule to trigger monthly-alert lambda function.
variable "cron" {
  #run every 30 mins
  default = "cron(0/30 * * * ? *)"
